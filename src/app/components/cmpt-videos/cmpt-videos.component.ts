...
@Component({
  selector: 'app-cmpt-videos',
  templateUrl: './cmpt-videos.component.html',
  styleUrls: ['./cmpt-videos.component.css']
})
export class CmptVideosComponent implements OnInit {
  ...
  generateExcelwww() {
      this.excelService.generateExcelwww();
    }
  ...
  create() {
  this.Service.save(localStorage.getItem('id') || "", "VIDEO")
  .subscribe(
  (res: any) => {
  console.log(res);
  this.gets()
