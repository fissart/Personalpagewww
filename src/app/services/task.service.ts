...
updatefiles(user:string, curse: string ) {
  ...
  create_average( nota: string, teacher: string, user: string, curse:string, title:string, ciclo:string, credito:string, especialidad:string, year: number) {
    return this.http.post<any>(`${environment.apiURL}/api/AVERAGE`, { nota, teacher, user, curse, title, ciclo, credito, especialidad, year});
  }
